...
cmd_install() {
    ...
    for unit in morning-brief.service morning-brief.timer; do
        ...
        # Refuse to install if any sentinel survives on a non-comment
        # line. We strip `^\s*#` lines first because the documentation
        # block at the top of the unit intentionally mentions __FOO__
        # as a literal example — and that would otherwise self-trigger.
